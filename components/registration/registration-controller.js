...
import {
    closeNaerkontaktEnrollment,
    duplicateEventsFromNaerkontaktToIndeksering,
    mergeTransferedEventsAndAutogeneratedEvents,
    ...
    $scope.getAttributes = function (_mode) {
        ...
        if($scope.selectedProgram){
            AttributesFactory.getByProgram($scope.selectedProgram).then(function (atts) {
                ...
                $scope.attributes = setCustomShowOnAttributes($scope.attributes, $scope.selectedProgram.id);
                        ...
                        EnrollmentService.enroll(enrollment).then(function (enrollmentResponse) {
                            if(enrollmentResponse) {
                                ...
                                if (en.status === 'SUCCESS') {
                                    ...
                                    var dhis2Events;
                                    if(enableAutoTransferFromNaerkontakt && $scope.selectedEnrollment.program == INDEKSERING_PROGRAM_ID) {
                                        transferNotesFromNaerkontaktToIndeksering(enrollment, $scope.tei, $scope.selectedEnrollment.program, EnrollmentService, DHIS2EventFactory);
                                        var transferredEvents = duplicateEventsFromNaerkontaktToIndeksering(enrollment, $scope.tei, $scope.selectedEnrollment.program);
                                        var autoGeneratedEmptyEvents = EventUtils.autoGenerateEvents($scope.tei.trackedEntityInstance, $scope.selectedProgram, $scope.selectedOrgUnit, enrollment, avilableEvent);
                                        dhis2Events = {events: mergeTransferedEventsAndAutogeneratedEvents(transferredEvents, autoGeneratedEmptyEvents.events)};
                                        closeNaerkontaktEnrollment($scope.tei, EnrollmentService);
                                    } else {
                                        dhis2Events = EventUtils.autoGenerateEvents($scope.tei.trackedEntityInstance, $scope.selectedProgram, $scope.selectedOrgUnit, enrollment, avilableEvent);
                                    }
